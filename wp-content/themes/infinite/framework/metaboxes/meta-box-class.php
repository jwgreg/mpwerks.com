...
class AT_Meta_Box {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_template_directory_uri() . '/framework/metaboxes';
  ...
  public function show() {
    ...
    foreach ( $this->_fields as $field ) {
      ...
      if(isset($field['page_template'])){
        $tmpl = get_post_meta( $post->ID, '_wp_page_template', TRUE );
        if(empty($tmpl) || stristr($field['page_template'], $tmpl) == false) continue;
      }

      if(isset($field['dont_show_in'])){
        $tmpl = get_post_meta( $post->ID, '_wp_page_template', TRUE );
        if(!empty($tmpl) && stristr($field['dont_show_in'], $tmpl) != false) continue;
      }
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_wysiwyg( $field, $meta,$in_repeater = false ) {
    ...
    if ( $in_repeater )
      ...
      $id = $this->stripNumeric( strtolower( $field['id']) );
  ...
  public function show_field_taxonomy($field, $meta) {
    ...
    if ('checkbox_list' == $options['type']) {
      foreach ($terms as $term) {
        echo "<input type='checkbox' ".( isset($field['style'])? "style='{$field['style']}' " : '' )." class='at-tax-checkbox".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}[]' value='$term->slug' id='custom_tax_$term->slug'" . checked(in_array($term->slug, $meta), true, false) . " /> <label for='custom_tax_$term->slug'> $term->name </label><br/>";
      }
    }
    // select
    else {
   ...
   *  @param $repeater bool  is this a field inside a repeatr? true|false(default)
   */
  public function addService($id,$args,$repeater=false){
    $new_field = array('type' => 'service','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'Text Field');
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
