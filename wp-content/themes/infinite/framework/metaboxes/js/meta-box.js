...
jQuery(document).ready(function($) {
  ...
  var show_services_select = $('select[name=infinite_meta_show_services]');

  if(show_services_select.val() == 'Yes'){
    show_services_select.closest('tr').next('tr').show();
  }else{
    show_services_select.closest('tr').next('tr').hide();
  }


  show_services_select.change(function(){
    var opt = $(this).val();
    if(opt == 'Yes'){
  ...
  $('select[id^=infinite_meta_services_info]').change(function(){
    var cols = $(this).val();

    if(cols == 2){
      $('#metabox-service-container-3, #metabox-service-container-4').hide();
    }else if(cols == 3){
      $('#metabox-service-container-3').show();
      $('#metabox-service-container-4').hide();
    }else if(cols == 4){
      $('#metabox-service-container-3, #metabox-service-container-4').show();
    }

  });

  $('.metabox-select-icon').click(function(e){
    e.preventDefault();
    var that = $(this);
    that.next('.metabox-icon-container').slideToggle();
  });

  $('.metabox-preview-icon').click(function(e){
    e.preventDefault();
    var that = $(this);
    that.parent().siblings('input[type=hidden]').val(that.attr('title'));
    that.parent().siblings('.metabox-icon').removeAttr('class').addClass('metabox-icon ' +  that.attr('title'));
